Imports System.Data
Partial Class Secured_UserAdmin_adminUserPermission
    ...
    Protected Sub cmdGVUpdate(ByVal sender As Object, ByVal e As CommandEventArgs)
        Response.Redirect("adminUserPermissionAdd.aspx?nuserid=" + e.CommandArgument.ToString() + "&nuserroleId=" + _clsDB.Get_DB_Item("SELECT user_role_id FROM tbl_user_info WHERE user_id='" & e.CommandArgument.ToString() & "'").ToString)
    End Sub

    Protected Sub cmdGVSelect(ByVal sender As Object, ByVal e As CommandEventArgs)
        Session("AE_STATUS") = "EDIT"
        Session("USERPERMISSION_ID") = e.CommandArgument.ToString()
        Response.Redirect("adminUserPermissionAdd.aspx")
    End Sub
